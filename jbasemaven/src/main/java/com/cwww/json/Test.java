...
//        System.out.println(str2);
        ...
        //用于测试age解析标准数值类型以及字符串类型
        String str = "[{\"age\":10,\"name\":\"test\"},{\"age\":\"180\",\"name\":\"test2\"}]";

        final List<User> user = JSON.parseArray(str, User.class);

        final ObjectMapper mapper = new ObjectMapper();
        final List<User> user1 = mapper.readValue(str, new TypeReference<List<User>>() {});

        //JsonIterator 无法将字符串类型解析为标准数值类型（会抛出异常）
        final List<User> user2 = JsonIterator.deserialize(str, new TypeLiteral<List<User>>() {
        });
